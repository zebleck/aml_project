...
class Agent:
    ...
    def train_model(self):
        for i_episode in range(NUM_EPISODES):
            ...
            for t in range(1, NUM_STEPS):
                ...
                if action_length % action_max_length == 0:
                    ...
                    self.lockdowns.append(action_idx)
                action_length += 1
                ...
                new_susceptible_count = sim.get_statistics(kind='info')["Susceptible"]
                # next_state = sim.update(action)
                # new_susceptible_count = next_state[1]
                # next_state = torch.tensor(next_state, dtype=torch.float)
                reward = torch.tensor([new_susceptible_count], dtype=torch.float)
                reward_total += new_susceptible_count
            ...
            string = f"Episode: {i_episode}, Susceptible: {susceptible_count}, eps_t: {round(max(EPS_START-self.steps_done*EPS_DECAY, EPS_END), 2)}, rewards_total: {round(reward_total, 2)}"
            if len(self.lockdowns) >= 500:
                string += f", Lockdown Ratio: {round(np.sum(self.lockdowns[-500:]) / (500), 2)}           "
        ...
        np.save("simulation_30.npy", self.lockdowns)
