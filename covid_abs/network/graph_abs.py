...
def calc_contacts(population_size, x, y, isdead, contagion_distance):
    contacts = []
    for i in np.arange(0, population_size):
        for j in np.arange(i + 1, population_size):
            if isdead[i] or isdead[j]:
                continue

            if distance(x[i], y[i], x[j], y[j]) <= contagion_distance:
                contacts.append((i, j))
    return contacts
...
class GraphSimulation(Simulation):
    ...
    def execute(self):
        ...
        for agent in filter(lambda x: x.status != Status.Death, self.population):
            ...
            if agent.infected_status == InfectionSeverity.Asymptomatic:
                for bus in filter(lambda x: x != agent.employer, self.business):
                    if distance(agent.x, agent.y, bus.x, bus.y) <= self.business_distance:
                        bus.supply(agent)
        ...
        contacts = calc_contacts(self.population_size,
                                 [a.x for a in self.population],
                                 [a.y for a in self.population],
                                 [a.status == Status.Death for a in self.population],
                                 self.contagion_distance)
